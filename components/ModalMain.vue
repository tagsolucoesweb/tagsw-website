...
<div class="modal-main" :class="props.modal ? 'active' : ''">
    <div>
        <h2>{{$t("Helping your company")}}<br /> {{$t("reach the next level")}}</h2>
        <div v-if="step.current == 0">
            <h3>{{$t("Where do you preffer to talk?")}}</h3>
            <ul class="contact-list">
                <li><a class="wpp-link contact-list-item" href="https://wa.me/5516981485154" target="_blank" @click="GoToStep(2)">
        ...
        <EmailForm v-else-if="step.current == 1" @submit-form="(result) => HandleSubmitForm(result)" />
